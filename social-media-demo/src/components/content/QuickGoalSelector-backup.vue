...
const quickGoals: Goal[] = [
  {
    ...
    icon: '🚀',
    trending: true,
    engagement_boost: '+240%',
    tip: 'Focus on benefits, not features. Use social proof and customer testimonials.'
  },
  {
    value: 'educate',
    label: 'Educate',
    ...
    tip: 'Break complex topics into simple, actionable steps. Use visuals and examples.'
  },
  {
    value: 'community',
    label: 'Community',
    ...
    tip: 'Ask questions, share opinions, and encourage responses. Be authentic and relatable.'
  },
  {
    value: 'entertain',
    label: 'Entertain',
    icon: '🎭',
    trending: true,
    engagement_boost: '+380%',
    tip: 'Use trending formats, memes, and current events. Keep it light and shareable.'
  }
];

const getSelectedGoalTip = () => {
  const goal = quickGoals.find(g => g.value === selectedGoal.value);
  return goal?.tip || '';
};

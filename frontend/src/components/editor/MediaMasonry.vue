  ...
  <div class="media-masonry">
    ...
    >
      ...
      <div class="absolute top-2 left-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
        <label class="relative flex items-center justify-center w-6 h-6 bg-white/90 backdrop-blur-sm rounded-full shadow-sm cursor-pointer hover:bg-white transition-all">
          <input
            type="checkbox"
...
const getMediaItemStyle = (file: MediaItem) => {
  if (!file.width || !file.height || file.type !== 'image') {
    return {
      paddingBottom: '75%' // 4:3 aspect ratio fallback
    }
  }
  ...
  if (aspectRatio > 2) {
    // Very wide images: limit to 2:1 ratio
    paddingBottom = 50
  } else if (aspectRatio > 1.5) {
    // Wide landscape images: limit to 1.5:1 ratio  
    paddingBottom = 66.67
  } else if (aspectRatio > 1) {
    // Landscape images: use actual ratio
    paddingBottom = (1 / aspectRatio) * 100
  } else if (aspectRatio > 0.5) {
    // Portrait images: use actual ratio
    paddingBottom = (1 / aspectRatio) * 100
  } else {
    // Very tall images: limit to 1:2 ratio
    paddingBottom = 200
  }
  
  // Ensure reasonable bounds - larger minimum for better visibility
  paddingBottom = Math.max(75, Math.min(220, paddingBottom))

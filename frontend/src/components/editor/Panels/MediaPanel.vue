<template>
  <div class="flex flex-col h-full bg-secondary-50 dark:bg-secondary-800">
    <!-- Header -->
    <div class="p-4 border-b border-secondary-200 dark:border-secondary-700 bg-white dark:bg-secondary-900">
      <h3 class="text-lg font-semibold text-secondary-900 dark:text-secondary-100">Stock Media</h3>
      <p class="text-sm text-secondary-600 dark:text-secondary-400 mt-1">Browse photos, icons, and shapes</p>
    </div>
      ...
      <div class="space-y-3">
        ...
        <div class="relative">
          <input
            ...
            class="w-full pl-10 pr-10 py-3 border border-secondary-200 dark:border-secondary-700 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200 bg-secondary-50 dark:bg-secondary-700 focus:bg-white dark:focus:bg-secondary-600 text-secondary-900 dark:text-secondary-100"
        ...
        <!-- Search Stats -->
        <div v-if="searchQuery && !isCurrentTabLoading" class="flex items-center justify-between text-xs text-secondary-500 dark:text-secondary-400">
    ...
    <div class="sticky top-0 z-10 bg-secondary-50 dark:bg-secondary-800 border-b border-secondary-200 dark:border-secondary-700">
      <div class="px-4 py-3">
        <div class="flex space-x-1 bg-secondary-100 dark:bg-secondary-700 rounded-lg p-1">
          <button
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <div v-if="activeTab === 'photos'" class="p-4">
        ...
        <div v-if="isLoadingPhotos && filteredPhotos.length === 0" class="space-y-4">
          <div class="text-center py-8">
            ...
            <p class="mt-2 text-sm text-secondary-600 dark:text-secondary-300">Loading photos...</p>
          </div>
          <!-- Loading skeleton -->
        ...
        <div v-else-if="filteredPhotos.length === 0 && !isLoadingPhotos" class="text-center py-12">
          ...
          <h3 class="mt-2 text-sm font-medium text-secondary-900 dark:text-secondary-100">No photos found</h3>
        ...
        <div v-else class="space-y-4">
          ...
          <div v-if="searchQuery" class="flex items-center justify-between">
            <h4 class="text-sm font-medium text-secondary-900 dark:text-secondary-100">{{ filteredPhotos.length }} Photos</h4>
            <button
              @click="clearSearch"
          ...
          <div v-if="hasMorePhotos && !isLoadingPhotos" class="pt-2">
            <button
              ...
              class="w-full py-3 px-4 bg-secondary-50 hover:bg-secondary-100 dark:bg-secondary-700 dark:hover:bg-secondary-600 border border-secondary-200 hover:border-secondary-300 dark:border-secondary-600 dark:hover:border-secondary-500 rounded-lg transition-all duration-200 text-sm font-medium text-secondary-700 hover:text-secondary-900 dark:text-secondary-300 dark:hover:text-secondary-100 flex items-center justify-center space-x-2"
            >
          ...
          <div v-if="isLoadingPhotos && filteredPhotos.length > 0" class="text-center py-4">
            <div class="inline-flex items-center space-x-2">
              ...
              <span class="text-sm text-secondary-600 dark:text-secondary-300">Loading more photos...</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'icons'" class="p-4">
        ...
        <div v-if="isLoadingIcons && filteredIcons.length === 0" class="space-y-4">
          <div class="text-center py-8">
            ...
            <p class="mt-2 text-sm text-secondary-600 dark:text-secondary-300">Loading icons...</p>
          </div>
          <!-- Loading skeleton -->
          <div class="grid grid-cols-4 gap-3">
            <div v-for="i in 8" :key="i" class="aspect-square bg-secondary-200 dark:bg-secondary-700 rounded-lg animate-pulse"></div>
          </div>
        </div>
        ...
        <div v-else-if="filteredIcons.length === 0 && !isLoadingIcons" class="text-center py-12">
          ...
          <h3 class="mt-2 text-sm font-medium text-secondary-900 dark:text-secondary-100">No icons found</h3>
          <p class="mt-1 text-sm text-secondary-500 dark:text-secondary-400">
        ...
        <div v-else class="space-y-4">
          <!-- Results header -->
          <div v-if="searchQuery" class="flex items-center justify-between">
            <h4 class="text-sm font-medium text-secondary-900 dark:text-secondary-100">{{ filteredIcons.length }} Icons</h4>
            <button
              @click="clearSearch"
              class="text-xs text-secondary-500 hover:text-secondary-700 dark:text-secondary-400 dark:hover:text-secondary-200 transition-colors"
            >
          ...
          <div class="grid grid-cols-4 gap-3">
            <div
              ...
              class="relative group cursor-pointer border border-secondary-200 dark:border-secondary-600 rounded-lg p-3 hover:border-primary-500 dark:hover:border-primary-400 hover:shadow-lg transition-all duration-200 bg-white dark:bg-secondary-800 transform hover:scale-105"
            >
          ...
          <div v-if="isLoadingIcons && filteredIcons.length > 0" class="text-center py-4">
            <div class="inline-flex items-center space-x-2">
              <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-primary-600 dark:border-primary-400"></div>
              <span class="text-sm text-secondary-600 dark:text-secondary-300">Loading more icons...</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'shapes'" class="p-4">
        ...
        <div v-if="isLoadingShapes && filteredShapes.length === 0" class="space-y-4">
          <div class="text-center py-8">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-600 dark:border-primary-400 mx-auto"></div>
            <p class="mt-2 text-sm text-secondary-600 dark:text-secondary-300">Loading shapes...</p>
          </div>
          <!-- Loading skeleton -->
          <div class="grid grid-cols-4 gap-3">
            <div v-for="i in 8" :key="i" class="aspect-square bg-secondary-200 dark:bg-secondary-700 rounded-lg animate-pulse"></div>
        ...
        <div v-else-if="filteredShapes.length === 0 && !isLoadingShapes" class="text-center py-12">
          ...
          <h3 class="mt-2 text-sm font-medium text-secondary-900 dark:text-secondary-100">No shapes found</h3>
          <p class="mt-1 text-sm text-secondary-500 dark:text-secondary-400">
        ...
        <div v-else class="space-y-4">
          <!-- Results header -->
          <div v-if="searchQuery" class="flex items-center justify-between">
            <h4 class="text-sm font-medium text-secondary-900 dark:text-secondary-100">{{ filteredShapes.length }} Shapes</h4>
            <button
              @click="clearSearch"
              class="text-xs text-secondary-500 hover:text-secondary-700 dark:text-secondary-400 dark:hover:text-secondary-200 transition-colors"
            >
          ...
          <div class="grid grid-cols-4 gap-3">
            ...
            >
              ...
              <div class="absolute inset-0 bg-primary-50 dark:bg-primary-900/20 opacity-0 group-hover:opacity-50 transition-all rounded-lg"></div>
              <!-- Shape name -->
              <p class="text-xs text-center text-secondary-600 dark:text-secondary-300 mt-1 truncate group-hover:text-secondary-900 dark:group-hover:text-secondary-100 transition-colors">{{ shape.alt }}</p>
            </div>
          </div>
          ...
          <div v-if="isLoadingShapes && filteredShapes.length > 0" class="text-center py-4">
            <div class="inline-flex items-center space-x-2">
              <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-primary-600 dark:border-primary-400"></div>
              <span class="text-sm text-secondary-600 dark:text-secondary-300">Loading more shapes...</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div
      v-if="tooltipVisible"
      class="fixed bg-secondary-900 dark:bg-secondary-700 text-white dark:text-secondary-100 text-xs px-2 py-1 rounded whitespace-nowrap z-[9999] pointer-events-none shadow-lg"
      :style="{

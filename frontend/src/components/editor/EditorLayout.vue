<template>
  <div class="h-screen bg-gray-50 dark:bg-gray-900 flex overflow-hidden">
    ...
    <div class="flex-1 flex flex-col min-h-0 h-full">
      ...
      <div class="flex-1 flex min-h-0 h-full">
        ...
        <div 
          ...
          class="w-80 bg-gray-50 dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full"
        >
          <!-- Panel Header -->
          <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between flex-shrink-0 bg-white dark:bg-gray-900">
    ...
    <div v-if="isLoadingTemplate" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-900 rounded-lg p-6 max-w-sm mx-4 text-center shadow-2xl border border-gray-200 dark:border-gray-700">
    ...
    <div v-if="getUnsyncedLayers.length > 0 || failedSyncLayers.length > 0" 
         class="fixed top-4 right-4 z-40 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-3 max-w-sm">
      <div class="flex items-start space-x-3">
        <!-- Status Icon -->
        <div class="flex-shrink-0">
          <div v-if="failedSyncLayers.length > 0" class="w-5 h-5 bg-danger-100 dark:bg-danger-900/20 rounded-full flex items-center justify-center">
          ...
          <div v-else class="w-5 h-5 bg-warning-100 dark:bg-warning-900/20 rounded-full flex items-center justify-center">
            <svg class="w-3 h-3 text-warning-600 dark:text-warning-400 animate-spin" fill="none" viewBox="0 0 24 24">
        ...
        <div class="flex-1 min-w-0">
          ...
          <button v-if="failedSyncLayers.length > 0" 
                  ...
                  class="mt-2 text-xs bg-danger-600 hover:bg-danger-700 text-white px-2 py-1 rounded transition-colors">
            Retry Sync
          </button>
        </div>
      </div>
